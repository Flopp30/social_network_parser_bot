...
@shared_task
def analyze_growth_easy():  # делим последнее на предпоследнее
    params = Parameter.objects.first()
    if not params:
        celery_logger.error("Couldn't analyze growth without params")
        return
    chats_id = (
        ...
        if len(results) == 2:
            ...
            if growth_ratio > params.alert_ratio:
                ...
                for chat_id in chats_id:
                    asyncio.run(HttpTelegramMessageSender.send_text_message(chat_id, message))


@shared_task
def analyze_growth_weight():  # ищем взвешенный рост
    ...
    for link in links:
        ...
        if weight_rate is not None and weight_rate > threshold:
            celery_logger.info(
